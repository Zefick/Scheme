...
impl Display for ParseErr {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ParseErr::WrongFP => write!(f, "Wrong floating point formatting"),
            ParseErr::UnclosedString => write!(f, "String literal didn't close"),
            ParseErr::Unexpected_EOF => write!(f, "Unexpected end of input"),
            ParseErr::Unexpected_EOF_AfterPars => {
                write!(f, "Unexpected end of input after opening parenthesis")
            }
            ParseErr::Unexpected_EOF_AfterDot => write!(f, "Unexpected end of input after a dot"),
...
impl Display for EvalErr {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            EvalErr::PairRequired(obj) => write!(f, "pair required but got {}", obj),
            // EvalErr::String(s) => write!(f, "{}", s),
            EvalErr::NeedAtLeastArgs(name, expected, actual) => write!(
                f,
                "\"{}\" need at least {} arguments, found {}",
            ...
            EvalErr::DivisionByZero() => write!(f, "Division by zero"),
            EvalErr::UnboundVariable(name) => write!(f, "Unbound variable {}", name),
            ...
            EvalErr::ArgumentDuplication(name) => write!(f, "Duplication of argument id: {}", name),
            EvalErr::ApplyNeedsProperList(obj) => write!(
                f,
                "'Apply' needs proper list as the last argument, got {}",
                obj
            ),
            EvalErr::UnequalMapLists() => write!(f, "List of wrong length for 'map'"),
            EvalErr::LetNeedSymbolForBinding(obj) => {
                write!(f, "Need symbol for 'let' binding, got {}", obj)
            }
            EvalErr::LetNeedListForBinding(obj) => write!(
                f,
                "'let' needs a list of length 2 for binding, got '{}'",
                obj
            ),
